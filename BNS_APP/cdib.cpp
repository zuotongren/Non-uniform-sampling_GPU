...
CSize CDib::GetDimensions()         //得到文件的大小值
{	
...
BOOL CDib::MakePalette()             //得到图片的调色板
{
...
//	TRACE("CDib::MakePalette -- m_nColorTableEntries = %d\n", m_nColorTableEntries);
	...
	if (pLogPal == NULL)
	{
		AfxMessageBox(_T("Error allocating memory for MakePalette"), MB_OK);
	}
...
BOOL CDib::Read(CFile* pFile)
{
	...
	try {
		...
		if (m_lpBMIH == NULL)
		{
			AfxMessageBox(_T("Error allocating memory for CDib,Read"), MB_OK);
		}
		...
		if (m_lpImage == NULL)
		{
			AfxMessageBox(_T("Error allocating memory for image CDib:Read"), MB_OK);
		}
